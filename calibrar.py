        ...
        if not ret:
            ...
            if time.time()-antes > tiempo:
                ...
                if ids is not None:
                    ...
                    if num>=8 and bboxsInt is not None and idsInt is not None: #si hay más de 8 marcadores captados
                        aruco.drawDetectedMarkers(frame, bboxs) #dibuja un marcador
                        allCorners.append(bboxsInt) #añade las esquinas
                        allIds.append(idsInt) #añade los ids
                        n = n + 1
